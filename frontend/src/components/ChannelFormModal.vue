  ...
  <div class="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
    <div class="bg-brand-surface text-brand-text rounded-lg p-6 w-full max-w-5xl shadow-xl overflow-y-auto max-h-[90vh]">
      ...
      <form @submit.prevent="save" class="space-y-6">
        ...
        <fieldset class="border border-brand-muted p-4 rounded-lg">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="form.network_name" type="text"
            ...
            <div>
              ...
              <input v-model.number="form.channel_number" type="number" min="1"
                     class="w-full p-2 rounded bg-brand-bg border border-brand-muted" required />
            </div>
            <div>
        ...
        <!-- Standard -->
        <div v-if="form.network_type === 'standard'" class="space-y-4">
          <fieldset class="border border-brand-muted p-4 rounded-lg">
            ...
            <div class="flex items-center space-x-2">
              <input type="checkbox" v-model="form.commercial_free" id="commercialFree" />
            ...
            <div>
              ...
              <select v-model="form.break_strategy"
                      class="w-full p-2 rounded bg-brand-bg border border-brand-muted">
                <option value="standard">Standard</option>
            ...
            <div>
              ...
              <select v-model.number="form.schedule_increment"
                      class="w-full p-2 rounded bg-brand-bg border border-brand-muted">
            ...
            <div v-for="field in fileFields" :key="field.key">
              ...
              <div class="flex space-x-2">
                <select v-model="form[field.key]"
                  ...
                  <option v-for="file in runtimeFiles[field.key]" :key="file" :value="file">
                    {{ file }}
                  </option>
                </select>
                <input
            ...
            <div>
              ...
              <select v-model="form.video_scramble_fx"
                      class="w-full p-2 rounded bg-brand-bg border border-brand-muted">
            ...
            <div>
              ...
              <input v-model="form.station_fx" type="text"
                     class="w-full p-2 rounded bg-brand-bg border border-brand-muted" />
            </div>
            <div>
              <label class="block text-sm">Panscan</label>
              <input v-model.number="form.panscan" type="number" step="0.1" min="0" max="2"
                     class="w-full p-2 rounded bg-brand-bg border border-brand-muted" />
            </div>
            <div class="flex items-center space-x-2">
              <input type="checkbox" v-model="form.video_keepaspect" id="keepAspect" />
            ...
            <div>
              ...
              <div v-for="(clip, i) in form.clip_shows" :key="i" class="flex space-x-2 mb-2">
                ...
                <button type="button" @click="form.clip_shows.splice(i,1)"
                        class="px-2 bg-brand-danger text-white rounded">✕</button>
              </div>
              <button type="button" @click="form.clip_shows.push({tags:'',duration:60})"
            ...
            <fieldset class="border border-brand-muted p-4 rounded-lg">
              ...
              <div v-for="(sf, i) in form.special_subfolders" :key="i" class="flex space-x-2 mb-2">
                ...
                <input v-model="sf.value" placeholder="Value"
                       class="flex-1 p-2 rounded bg-brand-bg border border-brand-muted" />
                <button type="button" @click="form.special_subfolders.splice(i,1)"
                        class="px-2 bg-brand-danger text-white rounded">✕</button>
              </div>
              <button type="button" @click="form.special_subfolders.push({type:'month',value:''})"
        ...
        <!-- Loop -->
        <fieldset v-if="form.network_type === 'loop'" class="border border-brand-muted p-4 rounded-lg">
          <legend class="px-2 text-lg font-semibold">Loop Channel</legend>
          <div class="flex items-center space-x-2">
            <input type="checkbox" v-model="form.loop_shuffle" id="shuffle" />
            <label for="shuffle" class="text-sm">Shuffle Playback</label>
          </div>
        </fieldset>

        <!-- Streaming -->
        <fieldset v-if="form.network_type === 'streaming'" class="border border-brand-muted p-4 rounded-lg">
          <legend class="px-2 text-lg font-semibold">Streaming Channel</legend>
          <div v-for="(url, i) in form.streams" :key="i" class="flex space-x-2 mb-2">
            <input v-model="form.streams[i]" type="text"
                   placeholder="Stream URL" class="flex-1 p-2 rounded bg-brand-bg border border-brand-muted" />
            <button type="button" @click="form.streams.splice(i,1)"
                    class="px-2 bg-brand-danger text-white rounded">✕</button>
          </div>
          <button type="button" @click="form.streams.push('')"
...
const runtimeFiles = reactive({
  off_air_video: [], sign_off_video: [], standby_image: [], be_right_back_media: []
})
...
const loadRuntimeFiles = async () => {
  try {
    const res = await axios.get(`${API}/api/runtime-files`)
...
const triggerFileInput = (key) => { const el = fileInputs[key]; if (el && el.click) el.click() }
const onFileSelect = (e, field) => { const file = e.target.files[0]; if (file) form[field] = `runtime/${file.name}` }

// === Watch props ===
watch(() => props.channel, async (ch) => {
  if (ch) {
    try {
      const res = await axios.get(`${API}/channels/baseline/${ch.config.network_type || "standard"}`)
      Object.assign(form, res.data.station_conf, JSON.parse(JSON.stringify(ch.config || {})))
    } catch {
      Object.assign(form, JSON.parse(JSON.stringify(ch.config || {})))
    }
  } else {
    try {
      const res = await axios.get(`${API}/channels/baseline/standard`)
      Object.assign(form, res.data.station_conf, { network_name: "" })
    } catch (err) {
...
const save = async () => {
  ...
  if (props.channel) {
    await store.updateChannel(props.channel.name, form)
  } else {
